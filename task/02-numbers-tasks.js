...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
    var distanceOnXAxis = Math.abs(x1) + Math.abs(x2),
        distanceOnYAxis = Math.abs(y1) + Math.abs(y2),
        distanceBetweenPoints = Math.sqrt(Math.pow(distanceOnXAxis, 2) + Math.pow(distanceOnYAxis, 2))
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    var angle = Math.atan2(Math.abs(x1*y2-y1*x2), x1*x2 + y1 * y2);
    return angle;
}
...
/**
 * Returns a diagonal length of the 3 given by its sides a,b,c.
 *
 * @param {number} a
 * @param {number} b
 * @param {number} c
